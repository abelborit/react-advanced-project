...
import { routesArrayModule } from "./routes";
...
const navRoutesDynamic = (): JSX.Element[] => {
  return routesArrayModule.map((routeElement) => (
...
export const NavigationLayoutModules = () => {
  return (
      ...
      <BrowserRouter>
        <div className="main-layout">
          <nav>
            ...
            {/* de forma directa */}
            {/* <ul>
            {routesArrayModule.map((routeElement) => (
              <li key={routeElement.to}>
                <NavLink
                  to={routeElement.to}
                  className={({ isActive }) => (isActive ? "nav-active" : "")}
          ...
          <div className="routes-layout">
            <Routes>
              ...
              <Route
                path="/*"
                element={<Navigate to={routesArrayModule[0].to} replace />}
              />
            </Routes>
          </div>
        </div>
